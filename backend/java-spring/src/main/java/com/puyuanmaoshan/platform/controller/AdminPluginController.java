...
public class AdminPluginController {
    ...
    public ApiResponse<Map<String, Object>> delete(@PathVariable("plugin_id") String pluginId,
        ...
        plugin.setStatus(0);
        plugin.setLifecycleStatus("deleted");
        pluginService.updateById(plugin);
    ...
    @PostMapping("/upload")
    public ApiResponse<?> uploadPlugin(
            ...
            @RequestHeader(value = "X-Request-Id", required = false) String requestId) {
        String reqId = RequestContextUtil.resolveRequestId(requestId, "req-admin-plugin-upload");
        log.info("========== [UploadPlugin] START ==========");
        log.info("filename: {}, size: {}, override: {}, operator: {}",
                file.getOriginalFilename(), file.getSize(), overrideExisting, operatorId);
        try {
            UploadPluginResponse response = pluginLifecycleService.uploadPlugin(file, overrideExisting, operatorId);
            log.info("[UploadPlugin] SUCCESS - pluginId: {}", response.pluginId());
            log.info("========== [UploadPlugin] END ==========");
            return ApiResponse.ok(response, reqId);
        } catch (Exception e) {
            log.error("========== [UploadPlugin] ERROR ==========");
            log.error("Exception: {}, Message: {}", e.getClass().getName(), e.getMessage());
            log.error("Stack trace:", e);
            e.printStackTrace(); // 临时添加，输出到控制台
            log.info("==========================================");
            return ApiResponse.error(400, "插件上传失败: " + e.getMessage(), null, reqId);
        }
    }

    // 临时测试方法
    @PostMapping("/upload-simple")
    public ApiResponse<String> uploadSimple(@RequestParam("file") MultipartFile file) {
        log.info("========== [UploadSimple] START ==========");
        log.info("文件名: {}, 大小: {}", file.getOriginalFilename(), file.getSize());
        log.info("========== [UploadSimple] END ==========");
        return ApiResponse.ok("收到文件: " + file.getOriginalFilename(), "upload-simple");
    }
