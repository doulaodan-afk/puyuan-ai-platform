...
import { getRequirementList, getRequirementDetail } from '../api/design-assistant'
import type { RequirementListItem } from '../api/design-assistant'
...
async function viewDetail(id: number) {
  router.push(`/design-requirement/${id}`)
}
...
<template>
  <div class="design-requirement-list page-container">
    <header class="header">
      <h1>我的设计需求</h1>
      <button @click="router.push('/design-requirement/create')" class="btn btn-primary">
        + 创建新需求
      </button>
    </header>
    ...
    <div v-else-if="requirements.length === 0" class="empty">
      <p>暂无需求</p>
      <button @click="router.push('/design-requirement/create')" class="btn btn-secondary">
        创建第一个需求
      </button>
    </div>
