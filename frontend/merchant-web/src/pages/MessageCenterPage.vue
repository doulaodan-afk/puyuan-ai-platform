...
import { getMessageList, markMessageRead, getUnreadCount } from '../api/design-assistant'
...
function goToRelated(message: any) {
  if (message.relatedId) {
    ...
    if (message.type === 'task') {
      router.push(`/task/${message.relatedId}`)
    }
  }
}
...
function startPolling() {
  refreshUnreadCount()
  pollTimer = window.setInterval(refreshUnreadCount, 30000) // 每 30 秒刷新一次
}
