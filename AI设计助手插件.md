...
| **面料特供商** | 与工作室深度合作、开发共创的特别供应商，管理面料库、查看需求数据 | 租户内（内部管理） |
...
面料特供商是**与工作室深度合作、开发共创的特别供应商**，划分到商家租户内部管理。不同于独立的外部面料商（属于独立租户），面料特供商作为商家租户的内部成员，具备以下功能：

- **面料库管理**：新增、编辑、上架下架面料信息，管理工作室专属面料库
- **需求数据查看**：查看团队内的设计需求数据，了解设计趋势与用布需求
- **任务协作**：在团队内查看和处理分配给自己的面料相关任务
- **AI 插件调用**：可调用 AI 设计助手插件能力，辅助面料管理与匹配
- **费用可视**：可查看工作室的 Token 消耗与计费情况

**角色代码**：`operator`（后端沿用），前端展示名称为「面料特供商」

**权限配置**：`["plugin_invoke", "billing_view", "fabric_manage"]`
...
- 在现有的 `ROLE_ADMIN`、`ROLE_TENANT` 之外，新增 `ROLE_DESIGNER`、`ROLE_DESIGN_ASSISTANT`、`ROLE_BOSS`、`ROLE_PATTERN_MAKER`、`ROLE_OPERATOR`、`ROLE_SUPPLIER`、`ROLE_PATTERN_SERVICE`
...
AI设计助手插件作为独立租户体系的工作空间，**首次进入插件时必须经过「选择工作室」这一动作**。一旦用户选择了工作室并进入，后续再次点击插件时直接进入上次选择的工作室，无需重复选择。用户可在插件内通过顶部导航栏的工作室切换菜单切换工作室。
...
用户点击「AI设计助手」插件入口
    │
    ├── 已有身份标记（localStorage 中 ai_design_role / ai_design_tenant_id / ai_design_identity_prefix 均存在）
    │       │
    │       ▼
    │   直接进入插件主界面（上次选择的工作室）
    │
    └── 无身份标记（首次进入或身份已失效）
            │
            ▼
        进入工作室选择/登录页面（identity-select）
            │
            ├── 上方：选择工作室（展示已有工作室列表，点击选中）
            │       │
            │       ▼
            │   点击「确认并进入」→ 进入插件主界面
            │
            └── 下方：创建新工作室
                ...
                填写工作室名称 → 点击「确认创建」
                    │
                    ▼
                创建成功 → 自动进入新工作室（无需再点「确认并进入」）
```

**简化理解**：点击插件 → 工作室选择页（上方选择已有工作室，下方创建新工作室）→ 二选一进入。

### 14.3 关键约束

| 约束项 | 说明 |
|--------|------|
| **首次必须选择** | 首次进入插件时必须经过工作室选择页面，不可跳过 |
| **记住选择** | 用户选择工作室后，身份标记写入 localStorage，再次点击插件时直接进入，无需重复选择 |
| **创建后自动进入** | 创建工作室成功后，自动调用 `confirmIdentity()` 进入新工作室，无需用户再点击「确认并进入」 |
| **插件内可切换** | 进入插件后，在 PluginLayout 顶部导航栏的下拉菜单中可切换工作室，切换后重新加载当前页面数据 |
| **退出后保留** | 用户离开插件返回主框架后，身份标记不清除，再次进入时直接进入上次的工作室 |

### 14.4 前端实现要点

- **路由守卫**：`installIdentityGuard` 检查 localStorage 中 `ai_design_identity_prefix`、`ai_design_tenant_id`、`ai_design_role` 是否存在，不存在则重定向到 `/plugins/ai-design-assistant/identity`
- **PluginsPage.vue**：点击插件入口时，如果已有身份标记则直接进入，不再清除标记强制重选
- **identity-select.vue**：工作室选择/登录页面，上方展示用户所属工作室列表供选择，下方提供创建新工作室入口
- **创建后行为**：创建成功 → 自动调用 `confirmIdentity()` → 写入 localStorage 并跳转到插件主界面
- **选择后行为**：选中已有工作室 → 点击「确认并进入」→ 调用 `confirmIdentity()` 写入 localStorage 并跳转

### 14.5 后端注意事项

- **TenantContextFilter SKIP_PATHS**：`/api/tenant/user/tenants`、`/api/tenant/create`、`/api/tenant/switch` 等接口**必须加入跳过路径**。这些接口在用户尚未选择工作室（无租户上下文）时被调用，若被 TenantContextFilter 拦截要求 `X-Tenant-Id` header，将导致工作室列表无法加载，形成死锁（用户无法选择工作室 → 无法获取租户上下文 → 无法调用接口 → 无法展示工作室列表）
- **snake_case 序列化**：`UserTenant` DTO 因全局 Jackson SNAKE_CASE 配置序列化为 `tenant_id`/`tenant_name` 等字段名，前端 `loadTenants()` 已用 `s.tenantId ?? s.tenant_id` 兜底处理

### 14.6 用户体验说明

此设计的目的是：
1. **减少重复操作**：用户已选择工作室后，再次进入无需重选，提升使用效率
2. **明确工作上下文**：首次进入时强制选择工作室，让用户清楚自己正在哪个工作室下操作
3. **创建即进入**：创建工作室后自动进入，避免用户创建完还要再点击确认的冗余操作
4. **灵活切换**：插件内提供工作室切换菜单，满足多工作室场景
